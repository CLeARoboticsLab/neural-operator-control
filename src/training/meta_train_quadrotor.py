"""SetONet meta-training for Quadrotor (varying dynamics, fixed goal).

Same structure as p2p_dynamics — branch encodes dynamics, trunk predicts controls.
Uses QuadrotorDataLoader for data loading.
"""
...
def meta_sample_batch(data_loader, data_type, num_tasks, K, N):
    """Sample meta-batch: support/query sampled from same dynamics config."""
    support_lists = [[] for _ in range(5)]
    query_lists = [[] for _ in range(5)]

    for _ in range(num_tasks):
        sup = data_loader.sample_task(data_type, K=K, N=N)
        qry = data_loader.sample_task(data_type, K=K, N=N)

        for lst, val in zip(support_lists, sup):
            lst.append(val)
        for lst, val in zip(query_lists, qry):
            lst.append(val)
...
def compute_setonet_loss(model, batch):
    ...
    # Expert trajectories include time; drop final timestep
...
def evaluate(model, data_loader, num_tasks, K, N, alpha,
             inner_update_fn, num_batches=10):
    total = 0.0
    for _ in range(num_batches):
        batch = meta_sample_batch(data_loader, "test", num_tasks, K, N)
...
def run_training(cfg, data_dir, output_dir, variant, seed=42, device="cpu"):
    ...
    num_iterations = meta_cfg.get("num_iterations", 1000)
    eval_interval = meta_cfg.get("eval_every", 100)
    num_eval_batches = meta_cfg.get("num_eval_batches", 10)

    print("=" * 60)
    print(f"SetONet Meta-Training — Quadrotor ({variant})")
    print("=" * 60)
    ...
    for iteration in range(num_iterations):
        batch = meta_sample_batch(data_loader, "train", num_tasks, K, N)
        ...
        if iteration % eval_interval == 0:
            eval_loss = evaluate(
                model, data_loader, num_tasks, K, N,
