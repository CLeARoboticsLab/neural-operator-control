...
Dispatches to the appropriate trainer based on the `generator` field
...
Usage (matches Makefile convention):
    python src/training/train.py \
        ...
        --data data/p2p_cost \
        --output checkpoints/p2p_cost/pretrained \
        --seed 42 --device cpu
"""
...
def main():
    parser = argparse.ArgumentParser(description="Train SetONet for an environment")
    ...
    print(f"Trainer:  {generator_name}")
    ...
    if generator_name == "p2p_cost":
        from src.training.train_p2p_cost import run_training
        run_training(cfg, data_dir=args.data, output_dir=args.output,
                     seed=args.seed, device=args.device)
    elif generator_name == "p2p_dynamics":
        from src.training.train_p2p_dynamics import run_training
        run_training(cfg, data_dir=args.data, output_dir=args.output,
                     seed=args.seed, device=args.device)
    elif generator_name == "quadrotor":
        from src.training.train_quadrotor import run_training
        run_training(cfg, data_dir=args.data, output_dir=args.output,
                     seed=args.seed, device=args.device)
